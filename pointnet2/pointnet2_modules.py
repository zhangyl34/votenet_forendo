...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
        ...
        self.normalize_xyz = normalize_xyz    # True
        self.ret_unique_cnt = ret_unique_cnt  # False

        if npoint is not None:  # this one
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        if not self.ret_unique_cnt:  # this one
            grouped_features, grouped_xyz = self.grouper(
                xyz, new_xyz, features
            )  # (B, C+3, npoint, nsample) (B, 3, npoint, nsample))
        else:
