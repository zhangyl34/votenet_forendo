...
def eval_det_multiprocessing(pred_all, gt_all, ovthresh=0.25, use_07_metric=False, get_iou_func=get_iou):
    """ Generic functions to compute precision/recall for object detection
        ...
        Output:
            ...
            ap: {classname: scalar}
    """
    pred = {}  # {scan_id(B*iterations): [(bbox,score)]}
    gt = {}    # {scan_id(B*iterations): [(bbox)]}
    for img_id in pred_all.keys():
        for bbox, score in pred_all[img_id]:
    ...
    for img_id in gt_all.keys():
        for bbox in gt_all[img_id]:
            if img_id not in gt:
                gt[img_id] = []
            gt[img_id].append(bbox)

    p = Pool(processes=10)
    ret_values = p.map(eval_det_cls_wrapper, [(pred, gt, ovthresh, use_07_metric, get_iou_func)])
    p.close()

    rec, prec, ap = ret_values[0]
    print(ap)
